...
public class ClientHandler implements Runnable{
    ...
    public void filterMessage(Message message) throws IOException {
        ...
        if(Objects.equals(message.getType(), "LOGIN")){
            ...
            serverModel.appendLogText("Un nuovo client: " + message.getSender() + " si e` connesso!");

        }else if(Objects.equals(message.getType(), "SENDMAIL")){
            serverModel.appendLogText( message.getSender() + " ha inviato una mail");
            Message response;
            ...
            if (reciversNotFound.isEmpty()){
                ...
                serverModel.appendLogText("Mail di " + message.getSender() + " inviata con successo");
            }else{
                response = new Message("ERRNOUSER",reciversNotFound,"SERVER",null);
                serverModel.appendLogText( message.getSender() + " ERRORE: non esistono questi destinatari : " + reciversNotFound + "");
            }
            ...
            serverModel.appendLogText("Richiesta di nuove Mail da " + message.getSender());
            Message response = new Message("GETMAILS","","SERVER",userMails);
            if (userMails.isEmpty()){
                serverModel.appendLogText("Nessuna nuova mail per " + message.getSender());
            }else{
                serverModel.appendLogText("Inviate nuove Mail a " + message.getSender());
            }
            SendMessage(response);
            ...
            serverModel.appendLogText(message.getSender() +" ha richiesto di cancellare una Mail");
            mailmanagement.cancelMail(message.getMail(),false, message.getSender());
            serverModel.appendLogText(message.getSender() + " la mail e` stata cancellata");
